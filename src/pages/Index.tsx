...
const Dashboard = () => {
  ...
  const [editDraft, setEditDraft] =
    useState<(PortfolioDraft & { id: string }) | null>(null);
  ...
  const handleImportMetadata = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // fallback description ignoré si non dispo
      }
      ...
      setMetadataError(
        error instanceof Error ? error.message : "Import impossible. Essayez manuellement."
      );
    } finally {
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-slate-950 px-6 text-center text-white">
        <div className="max-w-lg space-y-6 rounded-[3rem] border border-white/10 bg-white/5 p-12">
          ...
          <p className="text-lg text-slate-200/80">
            Connectez-vous avec le compte administrateur pour gérer le portfolio,
            les devis et les échanges clients.
          </p>
        </div>
      </div>
    );
                ...
                >
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <input
                      ...
                      onChange={(e) => setEditDraft((prev) => (prev ? { ...prev, tagline: e.target.value } : prev))}
                      placeholder="Tagline"
                      className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white focus:border-cyan-400 visual-accent-border focus:outline-none"
                    />
                  </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-4">
                    ...
                    <input
                      ...
                      type="number"
                      onChange={(e) =>
                        setEditDraft((prev) => (prev ? { ...prev, year: Number(e.target.value) } : prev))
                      }
                      placeholder="Année"
                      className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white focus:border-cyan-400 visual-accent-border focus:outline-none"
                    />
                    <input
                      ...
                      onChange={(e) => setEditDraft((prev) => (prev ? { ...prev, duration: e.target.value } : prev))}
                      placeholder="Durée"
                      className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white focus:border-cyan-400 visual-accent-border focus:outline-none"
                    />
                    <input
                      value={editDraft.videoUrl}
                      onChange={(e) => setEditDraft((prev) => (prev ? { ...prev, videoUrl: e.target.value } : prev))}
                  ...
                  <div className="grid gap-4 sm:grid-cols-3">
                    <input
                      ...
                      onChange={(e) => setEditDraft((prev) => (prev ? { ...prev, thumbnail: e.target.value } : prev))}
                      placeholder="Vignette"
                      className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white focus:border-cyan-400 visual-accent-border focus:outline-none"
                    />
                    <input
                      value={editDraft.aiTools}
                      onChange={(e) => setEditDraft((prev) => (prev ? { ...prev, aiTools: e.target.value } : prev))}
                      placeholder="IA Tools"
                      className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white focus:border-cyan-400 visual-accent-border focus:outline-none"
                    />
                    <input
                      value={editDraft.deliverables}
                      onChange={(e) => setEditDraft((prev) => (prev ? { ...prev, deliverables: e.target.value } : prev))}
                      placeholder="Livrables"
                      className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white focus:border-cyan-400 visual-accent-border focus:outline-none"
                    />
                  </div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditDraft((prev) => (prev ? { ...prev, socialStack: e.target.value } : prev))}
                    placeholder="Stack social"
                    className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white focus:border-cyan-400 visual-accent-border focus:outline-none"
                  />
