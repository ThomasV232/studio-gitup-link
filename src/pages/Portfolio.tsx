...
const Portfolio = () => {
  ...
  const categories = useMemo(
    () => ["Tous", ...Array.from(new Set(portfolioItems.map((item) => item.category)))],
    [portfolioItems],
  );
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-3xl space-y-6">
              ...
              <p className="text-lg text-slate-200/80">
                Chaque projet est une mini-série : humour, rythme, pipelines IA, data. Ajoutez, modifiez, supprimez vos
                cases depuis le dashboard.
              </p>
            </div>
          ...
          {filtered.length === 0 && (
            <div className="mt-16 rounded-[3rem] border border-white/10 bg-white/10 p-12 text-center text-sm text-slate-200/70">
              <p className="text-xs uppercase tracking-[0.3em] text-cyan-200/70 visual-accent-text">
                Filtre un peu trop spécifique
              </p>
              <p className="mt-4 text-2xl text-white">
                Ajoutez un nouveau projet depuis le dashboard pour remplir cette catégorie.
              </p>
              <Link
                to="/dashboard"
